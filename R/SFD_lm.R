#' Spatial First Differences (any direction)
#'
#' Spatial first difference (SFD) model for cross-sectional data estimated using the lm function on transformed data. You may specify the direction SFD are calculated in. Please see: Druckenmiller & Hsiang (2018).
...
#' @param balanced_df OPTIONAL; if needs_balancced==TRUE, specify a data.frame containing only those observations included in the balanced data. This data frame must include your observation variable.
...
SFD_lm <- function(spatial_df, n_channel,
  obs_var, dependent_var, independent_vars, rotation=0,
  needs_balanced = FALSE, balanced_df = NA,
  plot = TRUE){
  print(noquote("Computing SFD..."))
  ...
  rotateMap = function(object, angle) {
    boxpts = sp::SpatialPoints(t(sp::bbox(object)), proj4string = sp::CRS(proj4string(object)))
  ...
  if (nrow(sample) == 0) sample[1,] <- matrix(NA,1,(nvars+2))
  sample$row <- k
  ...
  if (plot == TRUE){
    ...
    plot(spatial_df)
    graphics::title("Adjacent Observations used for SFD")
    bound <- nrow(sample)
    for (n in 1:bound){
      pair <- sample[n,]
      adj <- pair$neighbor
  ...
  regression_results <- lm(
    as.formula(paste(paste(paste("d.", dependent_var, sep = ""), "~", explanatory_list))),
    data = sample)
